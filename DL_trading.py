...
class CNN_classsification1(nn.Module):
    ...
    def forward(self, x):
        ...
        return output
"""
class CNN_classsification2(nn.Module):
    def __init__(self):
        super(CNN_classsification2,self).__init__()
        #activations=nn.ModuleDict([['ELU',nn.ELU(alpha=1.0)],['ReLU',nn.ReLU()],['LeakyReLU',nn.LeakyReLU()]])
        ...
        self.depthwiseConv=nn.Sequential(
                 nn.Conv1d(16,32,kernel_size=5,stride=1,groups=16,bias=False),
        ...
        self.classify=nn.Sequential(
                 nn.Linear(in_features=800,out_features=8,bias=True)
                 )
...
def model_train(loader_train,loader_test):
    ...
    for epoch in range(EPOCH):
            ...
            if train_accuracy >= 99	 and profit > big_profit  :
                big_profit = profit
                torch.save(msresnet,"Newtable2016-201808.pkl")
