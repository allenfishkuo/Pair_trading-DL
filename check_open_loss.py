...
path_to_all = "C:/Users/Allen/pair_trading DL/all answer/"
...
def check_distributed():
   ...
   # print(test_label.shape)
    ...
    for i in range(len(unique)):
        if counts[i] >= 500 :
            plt.scatter(unique[i,0],unique[i,1], c ="red",alpha=0.4,label=' >500')
        elif counts[i] < 500 and counts[i] > 300:
            plt.scatter(unique[i,0],unique[i,1], c ="yellow",alpha=0.4,label='500> >300')
        else :
            plt.scatter(unique[i,0],unique[i,1], c ="blue",alpha=0.4,label='300')
